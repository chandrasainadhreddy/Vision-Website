...
// Cached detector options to reduce memory overhead
const faceDetectorOptions = new faceapi.TinyFaceDetectorOptions({ inputSize: 320, scoreThreshold: 0.3 })

// Eye Aspect Ratio (EAR) — used for blink-based liveness detection.
// A real human blink drops EAR below threshold; a static photo has constant EAR.
const calculateEAR = (eye: faceapi.Point[]): number => {
  // eye[0..5] are the 6 landmark points around one eye
...
export function TakeTestPage() {
  ...
  const accumulatedTimeRef = useRef(0) // tracks animation time including pauses
  const isPausedRef = useRef(false)       // ref: no re-render on pause/resume — intervals keep running
  const [showPauseOverlay, setShowPauseOverlay] = useState(false) // state: only for rendering the overlay UI
  ...
  // videoRef = hidden persistent video used exclusively for face-api detection (never unmounted during capture)
  const videoRef = useRef<HTMLVideoElement>(null)
  // displayVideoRef = the visible preview video shown in the UI (camera-permission & blue-dot-sequence)
  // Detection is run on displayVideoRef when available (browser always renders visible elements)
  const displayVideoRef = useRef<HTMLVideoElement>(null)
  // offscreenCanvasRef = used to snapshot video frames for reliable detection
  ...
  const closedFramesRef = useRef(0)   // consecutive frames where EAR dropped (eye closing)
  const blinkVerifiedRef = useRef(false) // liveness confirmed via blink
  // Dynamic EAR baseline — personal open-eye EAR calibrated per user session
  const earBaselineRef = useRef(0)         // user's personal open-eye EAR
  const earSamplesRef = useRef<number[]>([]) // samples collected during calibration
  ...
  const startTest = async (testType: TestType) => {
    ...
    try {
      ...
      if (data.status) {
        ...
        samplesRef.current = []
        // Reset liveness state for the new test session
  ...
  // attachStream: ref callback that immediately attaches the camera stream when a video element mounts.
  // This avoids the race condition where the detection interval fires before auto-reconnect can run.
  const attachStream = useCallback((el: HTMLVideoElement | null) => {
    displayVideoRef.current = el
    if (el && streamRef.current) {
      if (el.srcObject !== streamRef.current) {
        el.srcObject = streamRef.current
        el.play().catch(e => console.error('attachStream play error:', e))
      }
    } else if (!el) {
      // element unmounted - clear ref
      displayVideoRef.current = null
    }
  }, [])

  // Auto-Reconnect Video Stream: keep the visible display video in sync with the stream
  useEffect(() => {
    if (displayVideoRef.current && streamRef.current && displayVideoRef.current.srcObject !== streamRef.current) {
      displayVideoRef.current.srcObject = streamRef.current
      displayVideoRef.current.play().catch((e) => console.error('Auto-play error (display):', e))
    }
  })
        ...
        if (activeCaptureSteps.includes(currentStep)) {
          try {
            ...
            if (!cameraReady) {
              ...
              // Attach stream to visible display video
              if (displayVideoRef.current) {
                displayVideoRef.current.srcObject = stream
                displayVideoRef.current.play().catch((e) => console.error('Play error (display):', e))
              }
              setCameraReady(true)
  ...
  useEffect(() => {
    ...
    const detectFaces = async () => {
      ...
      try {
        ...
        const srcVideo = displayVideoRef.current ?? videoRef.current
        if (!srcVideo || srcVideo.readyState < 2) return // video not ready yet

        // Snapshot the current frame into an offscreen canvas for reliable detection
        const ofc = offscreenCanvasRef.current
        ofc.width = 320
        ofc.height = 240
        const octx = ofc.getContext('2d')
        if (!octx) return
        octx.drawImage(srcVideo, 0, 0, 320, 240)

        const detections = await faceapi
          .detectSingleFace(ofc, faceDetectorOptions)
          .withFaceLandmarks()
        ...
        if (detections) {
          ...
          // Phase 1: Calibrate the user's personal open-eye EAR (15 frames ~0.75s)
          ...
          if (currentStep === 'camera-permission' && !blinkVerifiedRef.current) {
            ...
            if (earBaselineRef.current === 0) {
              ...
              if (earSamplesRef.current.length >= CALIBRATION_SAMPLES) {
                // Compute baseline as the 80th-percentile value (ignores accidental partial blinks)
                const sorted = [...earSamplesRef.current].sort((a, b) => a - b)
                const p80idx = Math.floor(sorted.length * 0.80)
                earBaselineRef.current = sorted[p80idx]
                setDetectionStatus(`Calibrated ✓  Now please blink naturally once.`)
              } else {
                const remaining = CALIBRATION_SAMPLES - earSamplesRef.current.length
                setDetectionStatus(`Calibrating your eyes... (${remaining} frames left, keep eyes open)`)
              }
            } else {
              // ── Phase 2: Blink detection ──
              const blinkThreshold = earBaselineRef.current * BLINK_DROP_RATIO

              if (avgEAR < blinkThreshold) {
                closedFramesRef.current += 1
              } else {
                if (closedFramesRef.current >= 1 && closedFramesRef.current <= MAX_BLINK_FRAMES) {
                  // ✅ Real blink confirmed!
                  blinkVerifiedRef.current = true
                  setEyesDetected(true)
                  setDetectionStatus('✓ Blink detected! Liveness verified.')
                }
                closedFramesRef.current = 0
              }
              ...
              if (!blinkVerifiedRef.current) {
                setDetectionStatus(
                  closedFramesRef.current > 0
                    ? `Blinking... (EAR ${avgEAR.toFixed(3)} / baseline ${earBaselineRef.current.toFixed(3)})`
                    : `Face verified ✓  Please blink once. (baseline EAR: ${earBaselineRef.current.toFixed(3)})`
                )
              }
            }
          }
          ...
          } else if (currentStep === 'eyes-detection' || currentStep === 'camera-permission') {
            // Face lost — wait for a few frames before completely resetting.
            if (!blinkVerifiedRef.current) {
              // Grace period: if a baseline is calibrated, a missing face might be a blink
              if (earBaselineRef.current > 0 && lostFaceFramesRef.current <= 15) {
                closedFramesRef.current += 1;
              }

              if (lostFaceFramesRef.current > 15) {
                setEyesDetected(false)
                closedFramesRef.current = 0
                // Also reset calibration so it re-calibrates when face returns
                earBaselineRef.current = 0
                earSamplesRef.current = []
                setDetectionStatus('No face detected. Please position yourself in front of the camera.')
              }
            }
          }
        }

        if ((currentStep === 'camera-permission' || currentStep === 'eyes-detection') && canvas && detections) {
    ...
    detectionIntervalRef.current = setInterval(detectFaces, 50) // 20 FPS — fast enough to catch quick blinks
    return () => {
  ...
  useEffect(() => {
    ...
    if (currentStep === 'blue-dot-sequence') {
      // Reset warmup: block pausing for first 2 seconds to let the video stream re-attach and stabilize
      pauseAllowedRef.current = false
      const warmupTimer = setTimeout(() => { pauseAllowedRef.current = true }, 2000)
      accumulatedTimeRef.current = 0 // Reset at start of step
      const interval = setInterval(() => {
        if (isPausedRef.current) return // Skip countdown tick when paused
      ...
      if (selectedTest === 'RAN') {
        const jump = () => {
          if (!isPausedRef.current) {
            setDotPosition({ x: 15 + Math.random() * 70, y: 15 + Math.random() * 70, size: 20, opacity: 1 })
          }
        }
        jump()
        ...
        <main className="flex-1 overflow-y-auto p-8">
          ...
          {currentStep === 'camera-permission' && (
            <div className="max-w-3xl mx-auto bg-slate-900 rounded-3xl overflow-hidden shadow-2xl relative">
              <video ref={attachStream} autoPlay playsInline muted width="640" height="480" className={`w-full h-auto ${!cameraReady ? 'opacity-0 absolute -z-10' : 'opacity-100'}`} />
          ...
          {currentStep === 'blue-dot-sequence' && (
            <div className="max-w-4xl mx-auto relative bg-white rounded-3xl shadow-2xl border border-gray-200 aspect-[16/9] overflow-hidden">
              <video ref={attachStream} autoPlay playsInline muted width="160" height="120" className={`absolute bottom-4 left-4 rounded-lg border border-gray-300 z-30 shadow-lg ${!cameraReady ? 'opacity-0' : 'opacity-100'}`} />
              ...
              {showPauseOverlay && (
                <div className="absolute inset-0 bg-black/60 backdrop-blur-sm z-20 flex flex-col items-center justify-center text-white p-6 text-center">
                  <AlertCircle className="w-16 h-16 text-yellow-400 mb-4 animate-pulse" />
                  <h2 className="text-3xl font-bold mb-2">Eyes not detecting</h2>
                  <p className="text-xl text-gray-200">The test is paused. Please position your eyes clearly in front of the camera.</p>
                </div>
              )}
