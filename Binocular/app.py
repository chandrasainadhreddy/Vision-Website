...
def start_test():
    try:
        ...
        test_type_raw = data.get("test_type")

        if not user_id or not test_type_raw:
            return jsonify({"status": False, "message": "Missing user_id or test_type"})

        test_type = str(test_type_raw).lower()

        conn = get_db_connection()
        cur = conn.cursor()

        # Mark any old running tests for this user/type as abandoned
        cur.execute("""
            UPDATE tests
        ...
        # Always create a fresh new test
        cur.execute("""
...
def upload_eye_data():
    try:
        ...
        # 2. Save to upload table (if it exists)
        try:
            cur.executemany("""
                INSERT INTO upload
                (test_id,n,x,y,lx,ly,rx,ry)
            ...
            print(f"Note: Could not save to 'upload' table: {e}")
    ...
    try:
        ...
        return jsonify({
            ...
            "latest_test": {
                ...
                "test_type": latest["test_type"].upper() if latest else None
            } if latest else None,
...
def get_result():
    test_id_raw = request.args.get("test_id")
    
    if not test_id_raw or test_id_raw == "null" or test_id_raw == "undefined":
        return jsonify({"status": False, "message": "Invalid test_id provided"})

    try:
        test_id = int(test_id_raw)
    except ValueError:
        return jsonify({"status": False, "message": "test_id must be a number"})

    conn = get_db_connection()
    cur = conn.cursor()
    
    print(f"[DEBUG] Fetching results for test_id: {test_id}")
    
    cur.execute("""
        SELECT test_type, classification, score, percentage,
               stability, tracking, accuracy, reaction
    ...
    if not result:
        print(f"[DEBUG] No result found in DB for test_id: {test_id}")
        # List a few existing IDs to see what's in there
        cur.execute("SELECT test_id FROM results ORDER BY test_id DESC LIMIT 10")
        existing_ids = [r['test_id'] for r in cur.fetchall()]
        print(f"[DEBUG] Recent IDs in results table: {existing_ids}")
        
        conn.close()
        return jsonify({
            "status": False, 
            "message": f"Result for test #{test_id} not found. Available recent IDs: {existing_ids}"
        })
    
    conn.close()

    ai_notes = f"Our AI model analyzed your {result['test_type'] or 'test'}. "
    cls = result['classification'] or 'Normal'
    if cls == 'Normal':
        ai_notes += "The results show strong binocular stability and accurate tracking. Your eye movements follow the expected patterns for healthy visual coordination."
    elif cls == 'Mild Issue':
        ai_notes += "Some minor irregularities were detected in your tracking speed or accuracy. This might indicate slight fatigue or a minor coordination mismatch."
    else:
        ai_notes += "The analysis detected significant deviations in your eye movement patterns. We recommend focusing on vision therapy exercises to improve coordination and stability."

    return jsonify({
        "status": True,
        "data": {
            "test_type": (result["test_type"] or "N/A").upper(),
            "severity": cls,
            "score": float(result["percentage"] or 0),
            "stability": float(result["stability"] or 0),
            "tracking": float(result["tracking"] or 0),
            "accuracy": float(result["accuracy"] or 0),
            "reaction": float(result["reaction"] or 0),
            "ai_notes": ai_notes
        }
    })
